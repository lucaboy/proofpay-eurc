...
| [RFC 8259, JSON object interoperability](https://www.rfc-editor.org/rfc/rfc8259#section-4) | Duplicate object names produce unpredictable, parser-dependent mappings even when the bytes are otherwise valid JSON. | Independent evidence verification rejects duplicate decoded names at every nesting level before applying its exact-schema checks. | Evidence is deterministic JSON but is not claimed to implement the RFC 8785 JSON Canonicalization Scheme. |
| [TigerBeetle architecture](https://github.com/tigerbeetle/tigerbeetle/blob/main/docs/ARCHITECTURE.md#systems-thinking) and [Formance idempotency](https://docs.formance.com/modules/ledger/working-with/idempotency) | Production financial systems carry end-to-end idempotency identifiers and distinguish an exact retry from reuse with changed parameters. | The invoice ID is the business reference; a repeated create with identical immutable terms returns the original record, while changed terms fail closed. | ProofPay is a non-custodial verifier, not a distributed or double-entry core ledger. |
