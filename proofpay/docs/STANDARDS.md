...
| ZeroClaw's direct `agent --message` CLI path in stock v0.8.3 does not wire a receipt generator into the legacy loop, even though other runtime paths support tool receipts. | [ZeroClaw CLI loop](https://github.com/zeroclaw-labs/zeroclaw/blob/24476b71d33eb1672a9495a7ce3d155377a60ce8/crates/zeroclaw-runtime/src/agent/loop_.rs#L1937-L1978) | ProofPay makes no CLI-receipt claim. Its trace verifier requires a parsed native tool call and actual returned helper JSON; the video labels this as runtime evidence, not a signature or approval. |
