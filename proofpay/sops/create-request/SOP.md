...
4. **Safe handoff** — Present the Solana Pay URI, exact terms, reference, and digest. Label devnet as test-only. Tell the payer to review every field in their own wallet. State explicitly that ProofPay did not sign or submit a transaction and that the request remains pending until read-only reconciliation succeeds.
   - deny-tools: shell
